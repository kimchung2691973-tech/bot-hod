...
// Gửi embed + nút "Tạo yêu cầu mới" vào kênh hỗ trợ
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isModalSubmit() && interaction.customId === 'ticket_modal') {
    ...
    // Tạo permission cho kênh ticket
  ...
  if (interaction.isButton() && interaction.customId === 'close_ticket') {
    ...
    // Chỉ staff hoặc người tạo ticket mới được đóng
...
// Lệnh !ticketsetup để setup panel (chỉ staff dùng)
client.on('messageCreate', async (message) => {
  if (message.author.bot) return;
  if (message.content.trim().toLowerCase() !== '!ticketsetup') return;
  ...
  return message.reply('✅ Đã setup panel ticket!');
});
